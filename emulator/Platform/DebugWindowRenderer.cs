...
namespace JustinCredible.GalagaEmu
{
    public static class DebugWindowRenderer
    {
        ...
        public static void Render(IntPtr surface, DebuggerState state, string inputString, List<string> fileList, GalagaPCB pcb, CPUIdentifier cpuID, bool showAnnotatedDisassembly)
        {
            ...
            {
                RenderCpuStateAndDisassembly(surface, state, pcb, cpuID, showAnnotatedDisassembly);
            }

            SDL.SDL_RenderPresent(surface);
        }

        private static void RenderCpuStateAndDisassembly(IntPtr surface, DebuggerState state, GalagaPCB pcb, CPUIdentifier cpuID, bool showAnnotatedDisassembly)
        {
            ...
            if (state == DebuggerState.Breakpoint)
            {
                switch (cpuID)
                {
                    ...
                    case CPUIdentifier.CPU3_SoundProcessor:
                        ...
                        break;
                    default:
                        throw new NotImplementedException($"Unexpected CPU ID: {cpuID}");
                }
            }

            if (state == DebuggerState.Breakpoint && cpu != null)
            {
                pc = String.Format("{0:X4}", cpu.Registers.PC);
                sp = String.Format("{0:X4}", cpu.Registers.SP);
                regA = String.Format("{0:X2}", cpu.Registers.A);
                regB = String.Format("{0:X2}", cpu.Registers.B);
                regC = String.Format("{0:X2}", cpu.Registers.C);
                regD = String.Format("{0:X2}", cpu.Registers.D);
                regE = String.Format("{0:X2}", cpu.Registers.E);
                regH = String.Format("{0:X2}", cpu.Registers.H);
                regL = String.Format("{0:X2}", cpu.Registers.L);
                regDE = String.Format("{0:X4}", cpu.Registers.DE);
                regHL = String.Format("{0:X4}", cpu.Registers.HL);
                regIX = String.Format("{0:X2}", cpu.Registers.IX);
                regIY = String.Format("{0:X2}", cpu.Registers.IY);
                regF = String.Format("{0:X2}", cpu.Flags.ToByte());
                ...
                interrupts = $"{(cpu.InterruptsEnabled ? "Enabled" : "Disabled" )} (Mode {(int)cpu.InterruptMode})";
            }
            ...
            if (state == DebuggerState.Breakpoint)
            {
                ...
                FontRenderer.RenderString(surface, $"{COLOR_BRIGHT_WHITE}[F6] {COLOR_WHITE}Switch CPU     {COLOR_BRIGHT_WHITE}[F11] {COLOR_WHITE}Toggle Annotated Disassembly      {COLOR_BRIGHT_WHITE}[F12] {COLOR_WHITE}Print Last 50 Opcodes", 0, (commandsStartRow + 5) * ROW_HEIGHT);
            }
            else if (state == DebuggerState.SingleStepping)
            {
        ...
        private static void RenderBreakpointEditor(IntPtr surface, string inputString, GalagaPCB pcb)
        {
        ...
        private static void RenderSavedStateFileBrowser(IntPtr surface, DebuggerState state, string inputString, List<string> fileList, GalagaPCB pcb)
        {
        ...
        private static void RenderInstructionHistory(IntPtr surface, GalagaPCB pcb)
        {
            FontRenderer.RenderString(surface, $"{COLOR_BRIGHT_WHITE}------------------------------[HISTORY]-----------------------------------------", 0, 0 * ROW_HEIGHT);

            // TODO: Support showing history for CPU2/3.
            var recentInstructions = FormatRecentInstructions(pcb._addressHistory, pcb._cpu1.Memory, 50);
